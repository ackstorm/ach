...
# ACH operator deployment is named `ach-operator` in the `ach-system`
# namespace (see config/manager/manager.yaml after ach-old port).
# LiteLLM upstream deployment name is `litellm` in `litellm-system`.
kubectl -n ach-system logs deploy/ach-operator \
  --tail=2000 --all-containers=true > "${OUT_DIR}/operator-full.log" 2>&1 || true

kubectl -n ach-system logs deploy/ach-platform-api \
  --tail=2000 --all-containers=true > "${OUT_DIR}/platform-api-full.log" 2>&1 || true

kubectl -n ach-system logs deploy/ach-forwarder \
  --tail=2000 --all-containers=true > "${OUT_DIR}/forwarder-full.log" 2>&1 || true
...
kubectl -n default get \
  environments,plugins,pluginmarketplaces,artifacts,prompts,backendidentitypolicies \
  -o yaml > "${OUT_DIR}/default-crs.yaml" 2>&1 || true

# Recent events across the namespaces involved in e2e.
for ns in default ach-system litellm-system toolhive-system mocks dev; do
