...
// /tmp/e2e-<timestamp>-<spec>-*.log when the current spec failed.
...
func DumpForensicsOnFail() {
	...
	prefix := filepath.Join("/tmp", "e2e-"+stamp+"-"+sanitize(rep.LeafNodeText))

	targets := []struct {
		name string
		args []string
	}{
		{"operator.log", []string{"-n", "ach-system", "logs", "deploy/ach-operator", "--tail=400"}},
