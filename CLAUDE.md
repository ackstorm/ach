...
`achagents` (operator-written read model for the gateway `/hook` route set +
...
| gateway         | `ach gateway`         | **Optional** edge reverse proxy — single-origin front for the HTTP surfaces (no auth, no /metrics, no /dex); disable via `gateway.enabled=false`, use per-service Ingress instead. **Optionally** (when `ACH_DB_URL` is set) also reads the `achagents` projection and serves `/hook/{ns}/{name}/…` to per-agent webhook Services — still no auth/no header rewrite; the `ach-agent` harness verifies HMAC on `/channels/{name}/events` |
...
- Webhook inbound: GitHub → Ingress → gateway `/hook/{ns}/{name}/channels/{ch}/events` → (prefix stripped) → `achagent-{name}.{ns}.svc:8080/channels/{ch}/events` → harness HMAC-verify + dedup + session_key. The operator publishes this URL on `ACHAgent.status.webhookURL` (full URL when `ACH_PUBLIC_BASE_URL` is set on the operator, else the path-only form) whenever the agent has a `webhook` channel.
