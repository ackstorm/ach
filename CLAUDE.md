...
**One install topology** (the former `identity` profile — platform-api +
forwarder in front of one LiteLLM host, no operator — was removed
...
|--------------|------------|------|
| operator        | `ach operator`        | Reconciles ACH CRDs; `Environment.spec.budget` → the `environment:<name>` tag budget (condition `BudgetSynced`; absent budget publishes no condition, and the §6.5 drain reaps the tag + its budget object unconditionally); hourly LiteLLM orphan-key reaper scoped to keys stamped `metadata.ach_issuer == ACH_BASE_URL` (required) — never another release's or foreign keys; managed set = every non-revoked row across `personal_keys`+`environment_keys` — suspended/expired/invalid `ek_` rows are never reaped, only a revoked row's lingering LiteLLM key is (that reap IS the revoke retry, D-23) |
| platform-api    | `ach platform-api`    | REST + `pk_`/`ek_` lifecycle (an `ek_` is minted ONLY into its Environment's deny-all shell team `ach-env-<env>` — no models, no object_permission, no access-group binding; a `pk_` is minted into the caller's per-user deny-all shell team `ach-user-<email>` (provisioned idempotently at mint; a duplicate answer then triggers one drift-repair — `GET /team/info`, and `POST /team/update` only when the team is ACH-owned AND drifted — the user shell's only repair path, since no reconciler owns it; failures are logged, never fatal to the login) with a matching key expiry — grants attach via the operator, not platform-api; see `references/litellm-permission-model.md`; `POST /platform/keys` (ek_ create, optional future-dated `expires_at` else 400 `invalid_argument`, enforced by ACH only — no LiteLLM `duration`; optional `budget: {max_budget, budget_duration?}` capping THAT key through its own `key:<id>` tag, written after the row commits — a refused write is 502 `litellm_rejected`, never a silently uncapped key) + `PATCH /platform/keys/{id}/budget` (owner-scoped like suspend/resume, 400 unless `max_budget` is present and >= 0, 409 on a revoked key, 204; ~10s to take effect through LiteLLM's tag cache) + `POST /platform/keys/{id}/suspend`/`/resume` (ek_ only, idempotent; suspend `DEL`s the resolver cache entry, ≤60s enforcement bound; resume 409 on revoked/expired, 403 `unauthorized_team` when access is lost) + `DELETE /platform/keys/{id}` (ek_ revoke from any state, idempotent; also caller-scoped pk self-revoke, owner==caller, NOT admin-gated, `?force=true` overrides the active-key 409 guard); combined read `GET /platform/keys` (five effective states active/suspended/expired/invalid/revoked, priority Revoked→Expired→Suspended→Invalid→Active, D-30 `invalid`=owner lost Environment access, `?status=` filters on the effective state, `reasons` subset of `suspended|expired|no_access|access_unverified`) + `GET /platform/admin/keys` (SQL-level status only, no Invalid derivation, `?status=` now also accepts `suspended`) + admin object inventory (read) + UI Objects API (write, Environment only — `/platform/objects`, G2) + `PATCH /platform/admin/users/{email}/budget` (admin-gated; sets that person's whole-footprint ceiling on their `user:<email>` tag — same body and rule as the ek_ route, 204, a never-logged-in user is created not refused; durable — a login never overwrites it) + admin runtime catalog read (`GET /platform/admin/runtime/{models,mcp-servers,a2a-agents,teams,catalog}`) + **OAuth 2.1 AS — the only login** (`/platform/oauth/{register,authorize,as-callback,device_authorization,device,token}`: DCR, code+PKCE S256 via Dex, RFC 8628 device grant for headless hosts (`ach-cli login --no-browser`), refresh rotation **re-validated at the IdP** — `offline_access` at Dex, ONE Dex refresh token per user in Valkey (Dex keeps one per user+client and a new login replaces it, so every tool of the user shares and rotates it), every ACH refresh replays it, a refusal (user disabled at Google/Azure…) ends all the user's sessions and revokes the oauth `pk_`, Dex unreachable is a 503; no `pk_` is minted at login; `provisionUser` SEEDS the chart-configured default ceiling (`platformApi.userDefaults` → `ACH_USER_MAX_BUDGET`/`ACH_USER_BUDGET_DURATION`) onto the caller's `user:<email>` tag at login, but only while that tag carries no budget (one `/tag/info` read first), so an admin's `PATCH /platform/admin/users/{email}/budget` survives later logins and changing the chart default does NOT retune anyone already seeded; both the read and the write are fail-loud AT THE CALLBACK. The same seed also runs from `ensureOAuthPK`, the funnel EVERY grant crosses (login, `/token` refresh, device grant, console revalidation): a user provisioned before the default existed only ever refreshes and would otherwise never be seeded at all. There it is BEST-EFFORT — this path carries token refresh, so failing it on a LiteLLM blip would sign out every active user at once; what is missing is a ceiling, not a credential, and the next grant retries. The no-clobber guard lives inside `upsertUserBudgetTag`, so an admin's ceiling is safe on every call site — one ceiling covers their pk_ and every ek_ they own; issues 1h Ed25519 JWTs signed with the mounted `ach-jwt-signing-keys` seed (`ACH_JWT_SECRET_DIR`); each user's JWT resolves to one `personal_keys` row with `purpose='oauth'`, re-minted at `/token` when LiteLLM no longer lists its key (one `GET /key/list` per issue); RFC 8414/9728 documents are served by the forwarder) + the **OpenCode SSO plugin** for that AS (`GET /platform/opencode-auth`, anonymous: an npm tarball built at start from the embedded `internal/platformapi/opencodeauth/plugin/` — `opencode plugin https://<ACH_BASE_URL>/platform/opencode-auth -g`; two login methods, browser loopback first (OpenWork and the CLI default to `methods[0]`) and the RFC 8628 device grant for a remote/headless host (URL opened in any browser, the plugin polls `/token`); its `node --test` runs inside `make test-unit`) + **web console session** (`/platform/console/session/{login,logout}`: the same AS in-process — a `Console` pending, cookie `__Host-ach_console` HttpOnly/Lax, revalidated at the IdP every AccessTTL through the shared Dex refresh token, logout deletes the web session ONLY) + the cookie→`KeyContext` adapter (`middleware.AuthnOptions.Cookie`: same handlers as a bearer; cookie + any API credential = 400 `ambiguous_credentials`; non-safe methods need `Sec-Fetch-Site: same-origin` or `Origin`==host, D-28) + `/platform/console/{bootstrap,capabilities}` (Personal scope = LiteLLM reads as the user's OWN key via `litellm.UserView`, public projections only, deny-all-only or empty = `provisioning` (both sentinels recognised — `no-default-models` since 2026-09-22, legacy `__deny_all__` still read for shells an older ACH wrote); Environment scope = the projection behind `environments.CallerMayRead`) + `/platform/console/{stats,latency}` (GET, pk_-only — same guard as `capabilities`, 401 `invalid_key_type` for an `ek_`; each folds the caller's OWN LiteLLM window into `observability.{Stats,Latency}Contract` plus a top-level `"data_scope":"user"` always, D-13/AC-16 — user-global, never Environment-scoped, and includes traffic sent through any `ek_` the same owner holds; `stats`'s per-key rows relabel an ACH-managed key's alias to its ek_ `name`, or `"personal"` for the pk_ row, and folds every row the owner's OWN key set does not account for (another product's key, a sibling ACH release's, one made by hand in LiteLLM) into a single `external (LiteLLM)` row — summed, never dropped, so the block still covers the whole user-global window; ownership is decided by the owner's key rows, not a metadata issuer check, since each release tracks its keys in its own DB; independent degradation — a failed prior-window read only drops `capabilities.deltas` (all `totals.deltas.*` null), a failed `/tag/info` read only degrades `budget.source` to `"unknown"` (the budget panel reports the caller's own `user:<email>` tag — the ceiling the forwarder enforces — read with the MASTER client, `source: "tag"`), a failed CURRENT-window read is a hard 502 `litellm_rejected` (4xx/`Auth401Error`) or 503 `litellm_unreachable` (transport/5xx) — never a master-key retry, §10.1; `latency`'s `/spend/logs/v2` fetch degrades to 200 `{"available":false,"reason":"unavailable"}` on a 401 `role=unknown`, or `"fetch_failed"` on any other error, never an HTTP error) + the console SPA at `/` (`internal/platformapi/console`, `go:embed all:dist`, chi `NotFound` fallback; "console not built" until `make ui-build`) + **OpenWork Den** (spec §12; `openwork.enabled`, off by default): `/openwork` handoff page (console cookie → single-use grant → `openwork://` deep link), Den API under `/api/den` and `/openwork/api/den`, no Cloud MCP, `connectEnabled=false` + **BIP-declared consent** (an MCP `BackendIdentityPolicy` may name one `consentBroker` + `consentAudience`; ACH probes the backend through the forwarder, performs one broker metadata/DCR/PKCE hop only when the outcome is `auth_required`, then re-probes before issuing the code; no broker code is redeemed) |
| forwarder       | `ach forwarder`       | JWT trust path, `/v1`/`/gemini`/`mcp`/`a2a` rewrite; anonymous `/.well-known/jwks.json` + **ACH-composed** RFC 8414 (`/.well-known/oauth-authorization-server`) + RFC 9728 (`/.well-known/oauth-protected-resource[/v1|/gemini|/mcp/<n>|/a2a/<n>]`) documents (LiteLLM's PRM is no longer relayed; both are served on the path's SHAPE and never on whether the named server exists — an unknown but well-formed `/mcp/<n>` still gets a 200 document, so the anonymous surface is not an existence oracle, D-15); Authn reads the **declared** credential slots (`forwarder.headers` → `ACH_CREDENTIAL_HEADERS`, in order; defaults `x-ach-key`, `x-api-key`): `mode: resolve` → pk_/ek_ resolved, header removed, the caller's own LiteLLM key forwarded as `x-litellm-api-key`; `mode: passthrough` → the backend's own key forwarded as it came + mirrored to `x-litellm-api-key`, no ACH identity; every other header passes as it came. `/gemini` ALSO rewrites the model name in the path (`forwarder.gemini.stripModelPrefixes` → `ACH_GEMINI_STRIP_MODEL_PREFIXES`, a JSON list, declared order, first match wins): LiteLLM registers a model as `<vendor>.<model>` while a native Gemini client addresses it by its bare Google name, so the listed prefixes are stripped from the segment AFTER `/models/` — anchored there precisely so an entry like `gemini/` cannot match ACH's own `/gemini` route prefix. Empty/unset = no rewrite. `/v1` has no equivalent and will not get one: the model travels in the JSON body there and the Director never reads `req.Body` (D-05 streaming) — use LiteLLM's own `model_group_alias` for that surface. Every authenticated request is **stamped with the LiteLLM budget tags** in the Director — `x-litellm-tags: user:<email>[,environment:<env>,key:<ek id>]` (ek_ gets all three, pk_ only `user:`, no ACH identity gets none), which is how ALL of ACH's spend ceilings reach `/v1`, `/gemini`, `/mcp`, `/a2a` and `/v2/model/info` alike; budgets live ONLY on tags, never on a LiteLLM team or user object (measured — see `references/litellm-permission-model.md` §15), and the FWD-06 `metadata.tags` body injection is gone. With no declared slot present, `Authorization: Bearer` is resolved only when it is ACH's own OAuth token (JWS-shaped; a JWS that does not verify is 401 + challenge so MCP clients re-auth) — anything else there (LiteLLM's UI bearer, a raw key) is not ours: forwarded untouched with no ACH identity (no precheck, no BIP JWT, no env tag), LiteLLM authenticates it. Nothing presented at all → 401 + `WWW-Authenticate: Bearer resource_metadata=…`. There is **no catch-all** (D-18): a path outside `/v1`, `/gemini`, `/mcp/<n>`, `/a2a/<n>`, `/.well-known` is 404 — LiteLLM's own surface (`/ui`, `/key/*`, `/health`…) lives on LiteLLM's own host, never behind ACH. One owned exception: `GET /v2/model/info` (credential required, forwarded like `/v1`) because ach-agent's `litellm_usage` cost source prices there with its `ek_`; OAuth JWTs have no consent scopes and there is no static scope gate; BIP JWTs are minted for pk_/OAuth callers only — ek_ gets no JWT and no Authorization upstream; the `EkOwnerGate` middleware re-checks D-30 owner Environment-access on every authenticated family (`/v1`, `/gemini`, `/mcp`, `/a2a`, `/v2/model/info`) for `ek_` callers only — access lost is 403 `unauthorized_team`, a TeamsResolver failure is 503 `litellm_unreachable`, never a verdict |
...
- `POST /platform/keys/{id}/suspend`/`/resume` → ACH-only state flip + resolver cache `DEL` (the backing LiteLLM key is never touched); `expired`/`invalid` are derived at read/use time from the clock and the D-30 owner-access check, never persisted
...
| Phase | Command | When |
|-------|---------|------|
| `make test-unit`        | pure-logic, ~10s warm (+ the OpenCode plugin `node --test`) | every iteration |
