...
9. **`deploy/kustomize` bundle** has lost its regeneration tooling (2026-07-13 audit) — either it's a supported install surface (restore tooling) or it should be deleted (Helm-only).
...
- **Nightly e2e in CI** — keep PR CI fast, but run the full e2e suite on a schedule against `main` so regressions surface within 24h instead of at the next local run. Middle ground between cost and coverage. *(Done 2026-07-13: `e2e` job added to `nightly.yml`.)*
- **Close the hydrate mtime race** — small, known fix; converts a documented data-loss footgun into a non-issue.
- **Decide the `deploy/kustomize` bundle's fate** — support it or delete it; unmaintained install surfaces are worse than absent ones.
