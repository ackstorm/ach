...
|---|---|---|
| `Makefile` (build-installer) | same | Originally fed the now-retired `kustomize-to-helm.sh` veneer; `build-installer` is now a standalone kustomize install bundle (`kubectl apply -f dist/install.yaml`). The `kustomize edit set image controller=controller:latest` pin is a no-op (config/manager already pins `ghcr.io/ackstorm/ach`), kept for kubebuilder-convention parity. (`build-installer` + the other stock kubebuilder deploy targets removed 2026-07-13 — no external consumer, zero doc/CI references; Helm is the sole deploy path.) |
...
| `.github/workflows/nightly.yml` | same | Ported with a sed rename `alitellm-operator` → `ach`; **deleted 2026-07-17** (unused). Do not re-sync it from upstream. |
