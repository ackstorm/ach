...
| `helm-sync` | A | Copy the chart's ONE mirrored surface: the generated CRDs (`config/crd/bases` → `deploy/helm/ach/crd-sources/`, minus the gated-off Plugin kinds). The Grafana dashboards under `deploy/helm/ach/dashboards/` are NOT mirrored — they live there. |
