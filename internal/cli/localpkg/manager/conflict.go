...
// ConflictAction records one resolved clash for the caller to report. The
// policy is the governed `env hydrate` --conflict enum (internal/cli/conflict)
// so the two install paths behave consistently. NOTE the local installer is
// stateful and sequential: the FIRST-installed resource keeps its natural
// name; a LATER install that clashes is the one prefixed (we never
// retroactively rename an already-installed file). Only MergeReplace writes
// can clash: additive merges combine rather than clobber.
type ConflictAction struct {
	Policy  conflict.Policy
	Path    string // the original (clashing) destination path
	NewPath string // the namespaced destination (conflict.Namespace only; else "")
...
// conflict.Refuse returns an error and no writes.
//
// Non-clashing writes and non-MergeReplace writes (additive merges) always pass
// through unchanged and record no action.
func ResolveConflicts(writes []PlannedWrite, owners map[string]string, policy conflict.Policy, plugin, root string) ([]PlannedWrite, []ConflictAction, error) {
