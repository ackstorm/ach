...
func TestMergeStrategies_ConfigTomlIsDeep(t *testing.T) {
	a := &Adapter{}
	got := a.MergeStrategies()
	if got[".codex/config.toml"] != adapter.MergeDeep {
		t.Errorf("MergeStrategies()[\".codex/config.toml\"] = %v, want MergeDeep", got[".codex/config.toml"])
	}
}

func TestResolveOutputContent_TomlRoundTrips(t *testing.T) {
	a := &Adapter{}
	m := buildManifest()
	ctx := adapter.WithCredential(context.Background(), "pk_demo")
	...
	gotBytes, err := a.ResolveOutputContent(ctx, m, ".codex/config.toml")
	if err != nil {
...
func TestResolveOutputContent_NilManifest_ReturnsNilNil(t *testing.T) {
	a := &Adapter{}
	got, err := a.ResolveOutputContent(context.Background(), nil, ".codex/config.toml")
	if err != nil {
		t.Fatalf("ResolveOutputContent: %v", err)
	}
	if got != nil {
