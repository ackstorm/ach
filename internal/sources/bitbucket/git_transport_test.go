...
func TestGitTransport_Bitbucket_HappyPath(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git binary not on PATH")
	}
	bare := setupBitbucketBareFixture(t)
...
func TestGitTransport_Bitbucket_NotModified(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git binary not on PATH")
	}
	bare := setupBitbucketBareFixture(t)
	head := bitbucketHeadSHA(t, bare)
...
func TestGitTransport_Bitbucket_ConstructCloneURL(t *testing.T) {
	f, _ := New(&achv1alpha1.BitbucketSource{
		Workspace: "acme",
		Repo:      "widgets",
		Ref:       "main",
...
func setupBitbucketBareFixture(t *testing.T) string {
	t.Helper()
	work := t.TempDir()
...
func bitbucketHeadSHA(t *testing.T, bare string) string {
	t.Helper()
