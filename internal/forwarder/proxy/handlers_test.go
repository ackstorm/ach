...
// Identity profile: /mcp and /a2a forward the resolved identity with no
// precheck (nil providers would panic if consulted) and no BIP JWT, even
// for a pk_ that a full profile would mint for.
func TestHandlerMCPAndA2A_IdentityProfileSkipsPrecheckAndJWT(t *testing.T) {
	for _, tc := range []struct {
		name string
		h    func(HandlerDeps) http.HandlerFunc
		path string
	}{
		{name: "mcp", h: HandlerMCP, path: "/mcp/server-x"},
		{name: "a2a", h: HandlerA2A, path: "/a2a/agent-x"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			upstream, rec := upstreamSpy()
			defer upstream.Close()
			signer := &mockSigner{returnToken: "ACH-TOKEN"}
			deps := mkDeps(t, upstream, signer, precheck.Deps{}, nil)
			deps.Identity = true
			kc := middleware.KeyContext{KeyType: keys.PrefixPk, OwnerEmail: "u@e"}
			r := requestWithKC(t, http.MethodPost, tc.path, kc, "{}")
			r.Header.Set("Authorization", "Bearer client-token")
			...
			w := httptest.NewRecorder()
			tc.h(deps)(w, r)
			if w.Code != http.StatusOK || rec.Calls() != 1 {
				t.Fatalf("status=%d upstream calls=%d", w.Code, rec.Calls())
			}
			if rec.LastAuth() != "" || signer.signCalls != 0 {
				t.Fatalf("upstream Authorization=%q signer calls=%d; want none", rec.LastAuth(), signer.signCalls)
			}
		})
	}
}
